              ...
              <div className="flex flex-wrap gap-3">
                {['Pizarra', 'Programación', 'Micrófonos extra', 'Gestión remota'].map(need => (
                  <button
                    ...
                    className={`px-4 py-2.5 rounded-xl border-2 font-medium transition-all ${config.needs.includes(need)
                      ? 'border-[#E94E1B] bg-[#E94E1B] text-white'
                      : 'border-neutral-300 text-neutral-700 hover:border-neutral-400'
                      }`}
                  >
...
function Wizard({ onClose, whatsappNumber }: any) {
  ...
  const handleSubmit = () => {
    setAlertModal({
      ...
      message: 'Un especialista Trendit te contactará en breve con tu solución Logitech.'
    });
  };
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  {['Meeting Rooms', 'Work Desks', 'Hot-desking', 'Mixto'].map(obj => (
                    <button
                      ...
                      className={`p-6 rounded-xl border-2 text-left transition-all ${formData.objetivo === obj
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  {['Huddle', 'Sala pequeña', 'Sala mediana', 'Sala grande', 'Escritorio', 'Multi-sede'].map(esc => (
                    <button
                      ...
                      className={`p-5 rounded-xl border-2 text-left transition-all ${formData.escenario === esc
                        ? 'border-[#E94E1B] bg-[#E94E1B]/5'
                        : 'border-neutral-200 hover:border-neutral-300'
                        }`}
                    >
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {['Teams', 'Zoom', 'Meet', 'Mixta'].map(plat => (
                    <button
                      ...
                      className={`px-6 py-4 rounded-xl border-2 transition-all ${formData.plataforma === plat
                        ? 'border-[#E94E1B] bg-[#E94E1B] text-white'
                        : 'border-neutral-200 hover:border-neutral-300'
                        }`}
                    >
                ...
                <div className="flex flex-wrap gap-3">
                  {['Tap Scheduler', 'Scribe', 'Sight', 'Sync', 'Servicios (Essential/Select)'].map(extra => (
                    <button
                      ...
                      className={`px-5 py-3 rounded-xl border-2 transition-all ${formData.extras.includes(extra)
                        ? 'border-[#E94E1B] bg-[#E94E1B] text-white'
                        : 'border-neutral-200 hover:border-neutral-300'
                        }`}
                    >
